...
pushd $BUILD/src
make clean
make WANT_STEAM=0 WANT_DISCORD=0 PLATFORM=linux64 PLATFORM_BIN=amd64 PLATFORM_BUILD=${PLATFORM_BUILD} PLATFORM_BRANCH=\"${PLATFORM_BRANCH}\" PLATFORM_REVISION=\"${PLATFORM_REVISION}\" CFLAGS=-m64 CXXFLAGS=-m64 LDFLAGS=-m64 -j$(nproc) install
popd
